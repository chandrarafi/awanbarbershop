...
$options = getopt('', ['force', 'booking::', 'debug']);
$force = isset($options['force']);
$specificBooking = $options['booking'] ?? null;
$debug = isset($options['debug']);

// Parse standard arguments jika tidak menggunakan getopt
if (!$force && isset($argv[1]) && $argv[1] == '--force') {
    $force = true;
}
if (!$specificBooking && isset($argv[2]) && strpos($argv[2], '--booking=') === 0) {
    $specificBooking = substr($argv[2], 10);
}
