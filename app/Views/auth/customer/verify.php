...
<body class="bg-gradient-to-br from-[#2C3E50] to-[#3498DB] min-h-screen flex items-center justify-center p-4">
    <div class="bg-white/10 backdrop-blur-md w-full max-w-md p-8 rounded-2xl shadow-xl">
        ...
        <div class="mt-4 text-center">
            <a href="<?= site_url('customer/login') ?>" class="text-gray-300 hover:text-white text-sm">
                <span class="inline-block transform rotate-180">➜</span> Kembali ke Login
            </a>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            const updateCooldownText = () => {
                if (cooldown > 0) {
                    $('#resendBtn').text(`Tunggu ${cooldown} detik`).prop('disabled', true).addClass('opacity-50 cursor-not-allowed');
                } else {
                    $('#resendBtn').text('Kirim ulang').prop('disabled', false).removeClass('opacity-50 cursor-not-allowed');
                }
            };
